...
数据流图的外部实体和数据存储**100% 隐藏在题干说明文字中**，通过词性分析法即可秒杀：

```mermaid
flowchart TD
    READ["阅读系统功能说明段落"] --> SPLIT{"句式语法成分拆解"}
    
    SPLIT -->|"动作的主语/发起者<br/>(不在系统内部但与系统交互)"| ENTITY["判定为【外部实体 (E)】<br/>• 人员角色: 客户、管理员、审核员<br/>• 外部系统: 银联支付网关、物流系统<br/>• 硬件设备: 温湿度传感器、扫码枪"]
    
    SPLIT -->|"动作的宾语/沉淀物<br/>(带有保存、记录、历史、档案)"| STORE["判定为【数据存储 (D)】<br/>• 数据库表: 订单表、商品信息库<br/>• 记录日志: 登录日志文件、操作流水<br/>• 物理台账: 盘点明细账本"]
```
