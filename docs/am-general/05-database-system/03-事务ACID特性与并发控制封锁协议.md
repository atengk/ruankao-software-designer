...
flowchart LR
    subgraph Anomalies["并发数据三大异常"]
        direction TB
        A1["丢失修改: 两个事务同时修改，后提交覆盖先提交"]
        A2["读脏数据: 事务读取了其他事务未提交且随后回滚的数据"]
        A3["不可重复读: 同一事务两次相同读取，因他人提交修改导致结果不一致"]
    end
    subgraph Protocols["三级封锁协议递进防御"]
        direction TB
        P1["一级封锁: 写加 X 锁至事务结束 ➔ 消除【丢失修改】"]
        P2["二级封锁: 读加 S 锁读完即释放 ➔ 消除【丢失修改 + 读脏数据】"]
        P3["三级封锁: 读加 S 锁至事务结束 ➔ 消除【丢失修改 + 读脏数据 + 不可重复读】"]
    end
    A1 -.防范.-> P1
    A2 -.防范.-> P2
    A3 -.防范.-> P3
